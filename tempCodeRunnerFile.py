...
augmented_color_dir = "./Dataset-Places2/augmented/color"  # Artırılmış renkli görüntüler
augmented_gray_dir = "./Dataset-Places2/augmented/grayscale"  # Artırılmış gri görüntüler
...
for img_name in os.listdir(train_color_dir):
    ...
    # 5 farklı artırılmış veri üret
    for i in range(5):
        aug_color = it_color.next()[0].astype(np.uint8)  # Artırılmış renkli görüntü
        aug_gray = it_gray.next()[0].squeeze().astype(np.uint8)  # Artırılmış gri görüntü
